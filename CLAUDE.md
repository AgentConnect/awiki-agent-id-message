...
- **identity.py**: `DIDIdentity` 数据类 + `create_identity()` 封装 ANP 的 `create_did_wba_document_with_key_binding()`，生成 secp256k1 密钥对，公钥 fingerprint 自动构造 key-bound DID 路径（k1_{fp}）+ DID 文档 + WBA proof
...
- **ANP >= 0.5.7** 是硬性依赖，提供 DID 和 E2EE 底层密码学实现
- **Python >= 3.10**
